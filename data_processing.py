...
def data_clean(df1, df2):
    try:
        ...
        df_city_sel = df1.select(upper(col('city')).alias('city'),
                                 df1.state_id,
        ...
        loggers.warning('Working on OLTP dataset and selecting couple of columns and renaming....')
        ...
        loggers.warning('Concat first and last name')
        ...
        loggers.warning('Now Checking for null values in all columns')

        df_presc_sel = df_presc_sel.select([count(when (isnan(c)| col(c).isNull(), c)).alias(c) for c in df_presc_sel.columns])

        loggers.warning('Drop the null values in respective columns.....')
        ...
        loggers.error('An error occurred at data_clean() method====', str(exp))

    else:
        loggers.warning('data_clean() method executed done, go  forward.....')
